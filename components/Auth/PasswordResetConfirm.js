...
import { PASSWORD_RESET_CONFIRM } from '../../utils/mutations'
...
const PasswordResetConfirm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (password !== passwordTwo) {
      ...
      const { errors, data } = await passwordResetConfirm({
        variables: { email, newPassword: password, code },
      })
      if (errors) {
        setErrorMessage(errors[0].message)
      } else {
        alert('Compte rénitialisé')
